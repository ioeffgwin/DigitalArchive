...
namespace DigitalArchive
{
    public class CatalogueItemUpdate 
    {
        ...
        public Boolean AddItem(string itemName, string itemPath, byte[] itemChecksum, string itemOwner, Boolean itemCopyright = default, Boolean itemGDPR = default)
        {
            /*
        ...
        protected int GetItemID(string itemName, string itemPath, byte[] itemChecksum)
        {
            int myItemID = 0;
            SQLiteConnection cat_conn = new SQLiteConnection(Globals.connCat);
            cat_conn.Open();

            string sql = "SELECT FROM tblItems itemID WHERE itemName = '" + itemName +"' " +
